...
# Copy of ship.py that is slightly altered to fit the specifications of the rocket game in TIY 12-4
...
class Ship:
    ...
    def update(self):
        ...
        if self.moving_right:
            self.image = pygame.image.load('images/shipRIGHT_64.bmp')
            self.image_fix()
            if self.dx < 20:
                self.dx += 1
        if self.moving_left:
            self.image = pygame.image.load('images/shipLEFT_64.bmp')
            self.image_fix()
            if self.dx > -20:
                self.dx += -1
        if self.moving_up:
            ...
            if self.dy > -20:
                self.dy += -1
        if self.moving_down:
            self.image = pygame.image.load('images/shipDOWN_64.bmp')
            self.image_fix()
            if self.dy < 20:
                self.dy += 1
